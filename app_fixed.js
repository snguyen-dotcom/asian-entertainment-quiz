// Complete Asian Entertainment Quiz App JavaScript - FIXED VERSION
...
QuizApp.prototype.loadLeaderboard = function() {
    ...
    // Get all scores with better error handling
    let allScores = [];
    Object.values(users).forEach(user => {
        if (user && user.scores && Array.isArray(user.scores)) {
            user.scores.forEach(score => {
                if (score && score.category && score.score !== undefined) {
    ...
    console.log('All scores found:', allScores.length);
    ...
    if (allScores.length === 0) {
        leaderboardList.innerHTML = `
            <div class="no-scores" style="text-align: center; padding: 3rem; color: #666;">
                <i class="fas fa-trophy" style="font-size: 4rem; color: #ddd; margin-bottom: 1rem; display: block;"></i>
                <h3 style="margin: 0 0 0.5rem 0; color: #999;">No scores available</h3>
                <p style="margin: 0; font-size: 0.9rem;">Be the first to take a quiz in this category!</p>
            </div>
        `;
        return;
    }
    
    console.log('Displaying top', Math.min(allScores.length, 20), 'scores');
    ...
    allScores.slice(0, 20).forEach((entry, index) => {
        ...
        entryDiv.innerHTML = `
            ...
            <div class="leaderboard-info">
                ...
                <div class="leaderboard-date" style="font-size: 0.8rem; color: #999;">${formattedDate}</div>
            </div>
